...
export function ChatMessagePost({ postId }: ChatMessagePostProps) {
  ...
  return (
    <Link
      to={`/post/${post.postId}`}
      className="mt-2 block w-full bg-muted hover:bg-muted border border-border rounded-2xl p-3 text-left transition-all duration-200 group max-w-sm shadow-none cursor-pointer"
    >
      {/* Header */}
      ...
      <div className="flex gap-3 items-start">
        ...
        {mediaUrl && (
          <div className="h-14 w-14 rounded-xl overflow-hidden shrink-0 border border-border/40 bg-muted">
            <img
              src={mediaUrl}
              alt="Post preview"
