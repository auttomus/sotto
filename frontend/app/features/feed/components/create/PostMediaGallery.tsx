...
export function PostMediaGallery({ files, removeFile, fileInputRef }: PostMediaGalleryProps) {
  if (files.length === 0) {
    return (
      <div 
        ...
        className="w-full aspect-video bg-muted rounded-md border-2 border-dashed border-border flex flex-col items-center justify-center gap-3 cursor-pointer hover:bg-muted/70 transition group mb-5"
      >
  ...
  return (
    <div className="flex gap-2 overflow-x-auto pb-4 mb-4">
      {files.map((file, i) => (
        <div key={i} className="relative w-32 h-32 shrink-0 rounded-md overflow-hidden border border-border group">
      ...
      {files.length < 5 && (
        <button 
          type="button"
          onClick={() => fileInputRef.current?.click()}
          className="w-32 h-32 shrink-0 rounded-md border-2 border-dashed border-border flex flex-col items-center justify-center hover:bg-muted transition"
        >
