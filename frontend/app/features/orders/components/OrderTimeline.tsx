    ...
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 w-full relative">
      <div className="p-4 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md z-10 hidden md:flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setRole("ALL")} className={`px-4 py-1.5 rounded-full text-sm font-medium cursor-pointer ${role === "ALL" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}`}>Semua</button>
          <button onClick={() => setRole("BUYER")} className={`px-4 py-1.5 rounded-full text-sm font-medium cursor-pointer ${role === "BUYER" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}`}>Pembelian</button>
          <button onClick={() => setRole("SELLER")} className={`px-4 py-1.5 rounded-full text-sm font-medium cursor-pointer ${role === "SELLER" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}`}>Penjualan</button>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2 md:hidden hide-scrollbar">
          <button onClick={() => setRole("ALL")} className={`px-4 py-1.5 rounded-full text-sm font-medium shrink-0 cursor-pointer ${role === "ALL" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}`}>Semua</button>
          <button onClick={() => setRole("BUYER")} className={`px-4 py-1.5 rounded-full text-sm font-medium shrink-0 cursor-pointer ${role === "BUYER" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}`}>Pembelian</button>
          <button onClick={() => setRole("SELLER")} className={`px-4 py-1.5 rounded-full text-sm font-medium shrink-0 cursor-pointer ${role === "SELLER" ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300"}`}>Penjualan</button>
        </div>
            ...
            orders.map((order: any) => {
              ...
              return (
                <Link 
                  ...
                  className="block bg-white dark:bg-gray-950 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-indigo-200 dark:hover:border-indigo-800 shadow-sm hover:shadow-md transition group cursor-pointer"
                >
