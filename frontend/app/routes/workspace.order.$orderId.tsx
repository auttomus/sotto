...
import { ArrowLeft, CheckCircle2, Clock, CheckCircle, Upload, AlertCircle, Loader2 } from "lucide-react";
import { Link, useParams } from "react-router";
import { Avatar } from "../components/ui/Avatar";
import { Button } from "../components/ui/Button";
import { useGetOrderDetailQuery } from "~/core/apollo/generated";
...
export default function OrderRoute() {
  ...
  const partner = isBuyer ? order.sellerAccount : order.buyerAccount;
  const partnerName = partner?.displayName || (isBuyer ? "Penjual" : "Pembeli");
  const avatarUrl = resolveMediaUrl(partner?.avatarObjectKey);
  const listingMediaUrl = order.listing?.media?.[0] ? resolveMediaUrl(order.listing.media[0].objectKey || order.listing.media[0].url) : null;
      ...
      <header className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shrink-0 sticky top-0 z-10">
        <div className="px-3 h-16 flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            <Avatar src={avatarUrl} size="sm" alt={partnerName} />
            <div className="flex flex-col">
              <span className="font-semibold text-gray-900 dark:text-gray-100 text-sm">Pesanan #{order.id.slice(0, 8)}</span>
              <span className="text-[11px] text-gray-500 dark:text-gray-400">{isBuyer ? "Penjual" : "Pembeli"}: {partnerName}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 pb-5">
          <div className="relative flex items-center justify-between w-full">
            ...
            {/* Active Line (Simplified based on status) */}
            <div 
              ...
              style={{ 
                width: order.status === 'COMPLETED' ? '100%' : 
                       ...
                       order.status === 'IN_PROGRESS' ? '33%' : '0%' 
              }}
            ></div>

            {/* Steps */}
            {[
              { id: "paid", label: "Dibayar", done: true, active: order.status === 'PENDING' },
              { id: "working", label: "Dikerjakan", done: ['DELIVERED', 'COMPLETED'].includes(order.status), active: order.status === 'IN_PROGRESS' },
              { id: "review", label: "Review", done: order.status === 'COMPLETED', active: order.status === 'DELIVERED' },
              { id: "completed", label: "Selesai", done: order.status === 'COMPLETED', active: order.status === 'COMPLETED' },
            ].map((step, idx) => (
        ...
        <div className="bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
          ...
          <div className="flex gap-3">
            <div className="h-16 w-16 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden shrink-0">
              {listingMediaUrl ? (
                <img src={listingMediaUrl} alt="Listing" className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-gray-400 text-xl">📦</div>
              )}
            </div>
            <div className="flex flex-col flex-1">
              <h4 className="font-semibold text-gray-800 dark:text-gray-200 text-sm">{order.listing?.title || "Penawaran"}</h4>
              <p className="font-bold text-indigo-600 dark:text-indigo-400 text-sm mt-1">Rp {order.agreedPrice?.toLocaleString('id-ID')}</p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 dark:text-blue-300 mt-1 leading-relaxed">
                  Penjual telah mengirimkan hasil kerja. Silakan periksa di ruang obrolan dan setujui jika sudah sesuai.
                </p>
              </div>
            </div>
          </div>
